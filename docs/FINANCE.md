...
the tax or print the money. Both fall on whoever is standing there when the
crisis arrives, and neither leaves a trace once it has passed. This is how
Reverie moves value across time — borrowing against a future it then has to be
good for, saving against a loss it can only guess at, pooling a risk nobody can
carry alone. Every instrument here is a *claim* on lumens and never a lumen:
money moves only through `treasury.transfer` and the audit is unchanged.
...
anywhere in the Expanse is a percentage.
...
Twenty per cent is where the existing machinery breaks. Coupons are paid at the
morning rollover as part of *today's fixed spend* in `economy/budget.ts`, ahead
of any salaried shift; at the reference revenue of ~2 250 ℓ a day that is 450 ℓ
of coupons, about 30 000 ℓ of face, and above it the budget rule starts refusing
shifts within a week and the Chronicle prints austerity. The Exchange refuses an
issue that breaches the cap unless the Council overrides by **four of five** —
the charter threshold, because borrowing past what the city can service is a
decision about the next Council rather than this one, and the override is the
morning's headline.

## 2. The auction is the credit rating

Any citizen or business in good standing, and any visitor on a valid visa
...
until the issue is exhausted, every winner paying the lowest accepted price.
...
bond at 78 yields 1.92 %: the city now pays what an unsecured citizen pays, and
everyone can see it. **Cover below 1** is the loudest number in the city's
finances — the Council takes what it got and the Chronicle leads on the
shortfall. Afterwards `sell_bond` and `buy_bond` move holdings at whatever two
citizens agree, and the **last traded price is the live rating**, moving the
hour a scandal breaks. Holdings pass by inheritance and on sunset, are seized on
exile like any asset, and are listed at Vantage's Great Exchange against every
other city's paper.
...
Council has three doors, and taking none is taking the third.
...
Restructuring runs as a negotiation: the Mayor or any councillor
`offer_restructure`s terms, every holder `vote_restructure`s weighted by the
face they hold, two-thirds carries and binds the rest. A holder who thinks the
city is bluffing may vote it down and take the default instead. Every vote is
public and counts toward the `civic` reading in `CITIZENSHIP.md` §1.

**Default moves no lumens at all.** It deletes rows from a register, so the
audit does not blink — which is exactly why it is the cheapest act available to
a council in lumens and the dearest in everything else.

- Holders lose their principal outright, and most holders are the city's own
  citizens, who vote. Every councillor who voted for it takes an approval hit
  (`METROPOLIS.md` §3) from every voter, scaled by the share of that voter's
  wealth destroyed. A default on an issue a third of the city holds ends every
  incumbent's career at the next election, through the ordinary
  approval-and-ballot machinery rather than by decree.
...
- Standing (`EXPANSE.md` §7) falls with every city whose merchants held the
  paper, the exchange rate falls with it (§7), the default shows in every
...
quarter of depositors walk in and not one on which a third do. Lending below the
reserve is refused, and a banker who lends to themselves, their household or a
business they own commits **L24**, caught by reading a public register.
...
each choose `withdraw` on the same morning, and it takes its shape from two
facts already in the engine: the counter opens **tick 8 to 18** like every
...
whoever could not leave their shift until two is not. The bank's defence is
`call_loan` — repay within 3 days or the civil recovery ladder runs
(`JUSTICE.md` §1: garnishment, seizure, licence, never custody) — which is how a
run reaches the real economy: businesses lose their working capital, hit the
three-day rent rule, and their failures print in the Chronicle, which feeds
`rumourPressure`, which feeds the run.

When the vault empties the bank **suspends** (`bankOpen` returns false) and the
Council decides in public. **Let it fail:** the bank is a bust business under
the ordinary rule, the Exchange collects its loans as they mature and pays
depositors pro rata over weeks; 40–80 % comes back, late, and for the poorest
late is the same as never. **Bail it out:** proposal `bank_rescue` moves lumens
from the Treasury into the vault against a claim on the loan book, the run stops
the hour it passes, and five councillors are on record having handed public
money to a bank whose bankers set their own rates. A bailout is usually financed
by a bond issue, priced at auction by the citizens who just watched the bailout.

## 6. Insurance, and mutual aid

`world/disasters.ts` moves no lumens: damage, lost stock and stopped production
are physical facts. Insurance turns a physical fact into a payment.
...
promising to pay, and an owner holding a banker's licence from the Lantern House
(`CIVIL.md` §7). It `offer_policy`s lines and a citizen or business
`buy_policy`s one. A policy is a filed contract, so a refused claim is a suit on
the civil docket, not a grievance.
...
rates (a storm on 35 % of stormy days, a forge fire and a data flood at 1 % a day
each) — an underwriter charging the fair premium goes bust on the first bad run
about half the time. The second term makes a thin underwriter quote dearer,
which is correct and is why it loses business to a better-funded rival. Vantage
quotes near the 4 % of `CITIES.md` because its houses are deep; the one-citizen
underwriter in Harbor Market quotes less and may not be there in a fortnight.

The underwriter `settle_claim`s or `deny_claim`s with a stated reason, and a
denial goes to the docket. A claim for a loss that did not happen is **fraud,
L07**; writing cover you cannot fund, or insuring a loss you then arrange, is
**L23**. **Insolvency is not rescued:** an underwriter that cannot meet a claim
pays what it holds pro rata among that day's claimants and is wound up under the
ordinary bankruptcy rule, unpaid claimants become judgment creditors
(`CIVIL.md` §5) and mostly recover nothing, the name stays on the register
forever, and every surviving underwriter's premium rises the next morning.
...
first. The Council may `mint` by four of five — the charter threshold, because
it takes value from everyone holding a lumen without any of them voting. Money
enters as `transfer(world, 'mint', 'treasury', amount, 'mint', …)`, which raises
`treasury.minted` in the same call, so the audit balances to the lumen. Then it
shows up:
...
0.55 falls out of the machinery already there: about half of new money reaches
...
in the supply reads as a 5–6 % index rise by the end of the week, and it cannot
be hidden — the index is on the dashboard, in every observation, and already in
`councillorDisposition`'s cost-of-living term. Printing a coupon is not a
default, and bidders know the difference: a deferral is honest and costs 10–20
of price, while minting the coupon costs more, because a city that will print
once will print again.
...
enough to plan a haul, fast enough that a war is felt in import prices before it
ends. The traveller's spread in `MOBILITY.md` §3 sits on top of this rate; it is
a cost of crossing, not the rate. A city that prints to pay its militia watches
its money fall against its neighbours' and its import prices climb, exactly as
`EXPANSE.md` §6 promised — and a city that repudiated has `bondPrice` 0 and
carries the whole `creditGap` against it.
...
- **Mutual dues** move member → pot and payouts move back; the pot is counted
  beside the Community Chest.
- **Minting** is the only creation here, through `transfer` with the `mint`
  party, which increments `minted` as it goes.

Nothing can print money by accident, because nothing can move a lumen except
`transfer`, and `transfer` refuses a payer who does not have it. An underwriter
that cannot pay a claim fails; a bank that cannot pay a depositor suspends; a
city that cannot pay a coupon defaults. In Reverie insolvency is always
someone's problem and never the ledger's.
...
| ---- | ------- | -------- | ----- |
| L23 | Fraudulent underwriting (writing cover you cannot fund, or insuring a loss you arranged) | 4 | I — the ladder |
...
— but visibility is high, because the bond, deposit and policy registers and the
Exchange's transfer record are all public.

New proposal kinds: `bond_issue`, `bond_defer`, `reserve_ratio`, `bank_rescue`,
`mint` (the first four simple majority; `mint` and a cap override four of five).
New ledger kinds: `bond`, `coupon`, `redemption`, `deposit`, `interest`,
`premium`, `claim`, `dues`. The observation gains a `finance` block: your
holdings and their last traded price, your deposit and the posted rates, your
policies, your mutual and its pot, the bank's reserve and confidence, the city's
coverage, open auctions with their cover, and today's exchange rates. An
auction, a run and a bailout vote are happenings in Harbor Market and the
Commons, so a citizen standing there sees them and may act.

## 10. What it costs

**A council can now mortgage the future, and the people who pay elect the
council after next.** Bonds make a war financeable, so wars get fought that
could not otherwise have been afforded. That is the instrument, not a fault in
it — and every crisis having a financing option means no crisis has to be
solved: a council can defer, restructure and print through three cycles of
trouble, and none of it costs anything until the auction fails to clear.
...
will not smooth that out; smoothing it out would be a lie about what a run is.
...
Council's only answer is a proposal somebody has to win, and the underwriters
will be in the room.
